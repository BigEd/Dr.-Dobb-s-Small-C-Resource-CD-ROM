...
int 
  moninit,  /* monitor keyword entercountered in module flag */
  private,  /* static keyword beging processed flag */
  entry;    /* entry keyword being processed flag */
...
initials(size, ident, dim) int size, ident, dim; {
  ...
  savedim = dim;
  if (private) {
    /* do not make static variable public */
    toseg(DATASEG);
    sprintf(tmpbuf, "DECLARE(_%s)", ssname);
    outline(tmpbuf);
    }
  else
    public(ident);
...
/* get task/interrupt function parameters */
getparms(param, count)
int *param; int count;
{
  int i, n;
  n = 0;
  for (i = 0; i < count; i++)
    param[i] = 0;
  while (match(")") == 0) {
    if (number(&param[n % count]) == INT) {
      blanks();
      match(",");
      }
    else {
      error("illegal task parameter");
      skip();
      }
    n++;
    if(endst()) break;
...
*/
dofunction()  {
  int mon, 			/* monitor keyword encountered flag */
  task, 			/* task keyword encountered flag */
  intr, 			/* interrupt keyword encountered flag */
  param[2];			/* task/interrupt function parameters */
  char fnname[NAMESIZE];	/* name of function being processed */
  char *ptr;
  ...
  entry = task = mon = intr = 0; /* reset keyword encountered flags */

  /* check keywords before function */
  ...
  if (match("entry")) {
    ...
    if (moninit)
	{
    	error("Multiple monitor initialization definition");
    	errflag = 0;
  ...
  if (task) {
    ...
    if (n > 1)
      ...
      error("missing interrupt number");
    }
  else {
  ...
  if (argstk && mon) 
	error("No arguments allowed for monitor initialization");
  while(argstk) {
  ...
  gen(ENTER, 0);
  if (strcmp(ssname, "main") == 0)
    /* generate Concurrent Small C initialization P-code */
    gen(CSCINIT, 0);
  else if (entry)
    /* generate Enter Monitor P-code */
    gen(ENTERM, 0);
  statement();
  if(lastst != STRETURN && lastst != STGOTO)
    {
    /* generate Exit Monitor P-code */
    if (entry) gen(EXITM, 0);
    gen(RETURN, 0);
    }
  if(litptr) {
    toseg(DATASEG);
  ...
  /* output monitor initialization table */
  if (mon) {
    sprintf(tmpbuf, "MNTRS(_%s)", fnname);
    outline(tmpbuf);
    }
  else if (task) {
    sprintf(tmpbuf, "TASKS(_%s, %d, %d)", fnname, param[0], param[1]);
    outline(tmpbuf);
    }
  else if (intr) {
    sprintf(tmpbuf, "INTRS(_%s, %d)", fnname, param[0]);
    outline(tmpbuf);
    }
  }
...
ask() {
  ...
  while(getarg(++i, line, LINESIZE, argcs, argvs) != EOF) {
    ...
    fputs("usage: csc [file]... [-m] [-a] [-p] [-l#] [-no]\n", stderr);
    abort(ERRCODE);
    }
  }
...
openfile() {        /* entire function revised */
  ...
  while(getarg(++filearg, pline, LINESIZE, argcs, argvs) != EOF) {
    ...
    outfn[j] = 0;
    sprintf(tmpbuf, "MODULENAME(%s)", outfn);
    outline(tmpbuf);
    files = YES;
    kill();
    return;
